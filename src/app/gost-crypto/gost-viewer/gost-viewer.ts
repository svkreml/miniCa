...
export class GostViewer {
    ...
    printASN1(value) {
      ...
      function process(block: ASN1Object, offset: number, ident: string) {
            ...
            ident = ident || '';
            // type name
            let typeName = block.tagClass === 1 ? 'Application_' + block.tagNumber :
                block.tagClass === 2 ? '[' + block.tagNumber.toString() + ']' : // Context
                    block.tagClass === 3 ? 'Private_' + block.tagNumber :
                        GostViewer.BERtypes[block.tagNumber] || 'Universal_' + block.tagNumber.toString();
    ...
    printSyntax(value, type) {
...

        {
            ...
            if (type) {
                return process(this.asn1[type].decode(value), undefined);
            } else
