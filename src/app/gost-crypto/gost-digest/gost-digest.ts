...
//  }
    ...
    constructor(public algorithm: AlgorithmIndentifier) {
        ...
        switch (algorithm.version || 2012) {
            case 1: // SHA-1
                this.digest = this.digestSHA1;
                this.bitLength = 160;
                break;
            case 1994:
                this.digest = this.digest94;
        ...
        switch (algorithm.mode || 'HASH') {
            ...
            case 'HMAC':
                ...
                break;
            case 'KDF':
                this.deriveKey = this.deriveKeyDefault;
                this.deriveBits = this.deriveBitsKDF;
                this.label = algorithm.label;
                this.context = algorithm.context;
                break;
            case 'PBKDF2':
                this.deriveKey = this.deriveKeyDefault;
                this.deriveBits = this.deriveBitsPBKDF2;
                this.generateKey = this.generateKeyDefault;
            ...
            case 'PFXKDF':
                this.deriveKey = this.deriveKeyDefault;
                this.deriveBits = this.deriveBitsPFXKDF;
                this.generateKey = this.generateKeyDefault;
                this.salt = algorithm.salt;
                this.iterations = algorithm.iterations || 2000;
                this.diversifier = algorithm.diversifier || 1;
            ...
            case 'CPKDF':
                this.deriveKey = this.deriveKeyDefault;
                this.deriveBits = this.deriveBitsCP;
                this.generateKey = this.generateKeyDefault;
                this.salt = algorithm.salt;
                this.iterations = algorithm.iterations || 2000;
                break;
            default:
                throw new Error('Algorithm mode ' + algorithm.mode + ' not supported');

        }
    }

    // Copy len values from s[sOfs] to d[dOfs]
    public static arraycopy(s, sOfs, d, dOfs, len) {
        for (let i = 0; i < len; i++) {
            d[dOfs + i] = s[sOfs + i];
        }
    }
...
//     }
    ...
    deriveBitsPFXKDF(baseKey, length): ArrayBuffer {
        ...
        for (let i = 0; i < c; i++) {
            ...
            GostDigest.arraycopy(D, 0, H, 0, v);
            GostDigest.arraycopy(I, 0, H, v, slen + plen);
            for (let j = 0; j < r; j++) {
    ...
    deriveBitsKDF(baseKey, length) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            result.set(new Uint8Array(this.signHMAC(baseKey, data), 0,
    ...
    deriveBitsPBKDF1(baseKey, length) {
        ...
        GostDigest.arraycopy(P, 0, T, 0, plen);
        GostDigest.arraycopy(S, 0, T, plen, slen);
        for (let i = 0; i < c; i++) {
            T = new Uint8Array(this.digest(T));
        }
        GostDigest.arraycopy(T, 0, DK, 0, dkLen);
        return DK.buffer;
    } // </editor-fold>
    ...
    deriveBitsCP(baseKey, length) {
        ...
        if (p) {
            GostDigest.arraycopy(p, 0, d, slen, plen);
        }
        ...
        for (let j = 0; j < iterations; j++) {
            ...
            GostDigest.arraycopy(s0, 0, d, 0, b);
            GostDigest.arraycopy(h, 0, d, b, l);
            GostDigest.arraycopy(s1, 0, d, b + l, b);
            GostDigest.arraycopy(h, 0, d, b + l + b, l);
            GostDigest.arraycopy(new Uint8Array(this.digest(d)), 0, k, 0, l);
        }
        for (let i = 0; i < l; i++) {
            s0[i] = k[i] ^ 0x36;
            s1[i] = k[i] ^ 0x5C;
        ...
        GostDigest.arraycopy(s0, 0, d, 0, l);
        GostDigest.arraycopy(salt, 0, d, l, slen);
        GostDigest.arraycopy(s1, 0, d, l + slen, l);
        if (p) {
            GostDigest.arraycopy(p, 0, d, l + slen + l, plen);
        }
