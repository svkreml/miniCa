...
export class GostCipher {
    ...
    * */
    ...
    public keySchedule: (k: Uint8Array | ArrayBuffer, e: boolean) => Uint32Array | Uint8Array | ArrayLike<number>;
    public processMAC: (key, s, d) => void;
    public keyMeshing: (k: Uint8Array, s: Uint8Array, i: number, key: ArrayBuffer | Uint32Array | Uint8Array | ArrayLike<number>, e: boolean) => ArrayBuffer | Uint32Array | Uint8Array | ArrayLike<number>;
    ...
    * */
    ...
    private static intArray(d: Uint8Array): Uint32Array {
        return new Uint32Array(GostCoding.buffer(d));
    }
    ...
    private static round(s: Uint8Array, m: Uint32Array, k: number): void {
    ...
    private static keySchedule89(k: Uint8Array, e: boolean): Uint32Array {
        const sch = new Uint32Array(32);
        const key = new Uint32Array(GostCoding.buffer(k));
        for (let i = 0; i < 8; i++) {
            sch[i] = key[i];
        }
        if (e) {
    ...
    private static keySchedule15(k: Uint8Array, e: boolean): Uint32Array {
        const sch = new Uint32Array(32);
        const key = new Uint32Array(GostCoding.buffer(k));
        for (let i = 0; i < 8; i++) {
    ...
    private static maskKey(mask, key, inverse, keySize) {
        const k = keySize / 4;
        const m32 = new Uint32Array(GostCoding.buffer(mask));
        const k32 = new Uint32Array(GostCoding.buffer(key));
        const r32 = new Uint32Array(k);
        if (inverse) {
            for (let i = 0; i < k; i++) {
    ...
    private process89(k: Uint32Array, d: Uint32Array, ofs: number): void {
        ofs = ofs || d.byteOffset;
        const s = this.sBox;
        const m = new Uint32Array(d.buffer, ofs, 2);
    ...
    private process15(k: Uint32Array, d: Uint32Array, ofs: number): void {
        ofs = ofs || d.byteOffset;
        const s = this.sBox;
        const m = new Uint32Array(d.buffer, ofs, 2);
    ...
    private processCTR89(k, d, iv): ArrayBuffer {
        ...
        const q = (cb - r) / b;
        const key = this.keySchedule(k, undefined);
        const syn = new Uint32Array(s.buffer);
    ...
    private processMAC89(key, s, d) {
        ...
        const sum = new Uint32Array(s.buffer);
    ...
    private diversifyKEK(kek: Uint8Array, ukm: Uint8Array): ArrayBuffer {
        ...
        for (let i = 0; i < n; i++) {
            ...
            k = new Uint32Array(this.encryptCFB(k, k, iv));
            //     D) i = i + 1
        }
        // 5) Let K(UKM) be K[8].
        return k;
    ...
    private keyMeshingCP(k: Uint8Array | ArrayBuffer, s: Uint8Array, i: number, key: Uint32Array, e: boolean): ArrayBuffer {
